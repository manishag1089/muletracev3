...
def load_txns():
    import glob
    parts = sorted(glob.glob(f"{DATA}/transactions_part*.parquet"))
    if parts:
        return pd.concat([pd.read_parquet(p) for p in parts], ignore_index=True)
    return pd.read_parquet(f"{DATA}/transactions.parquet")

@st.cache_resource(show_spinner=False)
def adjacency(edges):
    """Lightweight adjacency dicts — avoids holding a full nx graph in RAM."""
    fwd, rev = {}, {}
    for s, d, a, su in zip(edges["src"].values, edges["dst"].values,
                            edges["amount"].values, edges["is_suspicious"].values):
        fwd.setdefault(s, []).append((d, float(a), int(su)))
        rev.setdefault(d, []).append((s, float(a), int(su)))
    return fwd, rev

def ego(edges, focal, hops=2, max_nodes=110):
    fwd, rev = adjacency(edges)
    if focal not in fwd and focal not in rev:
        return nx.DiGraph(), {}
    seen, front = {focal}, {focal}
    for _ in range(hops):
        nxt = set()
        for n in front:
            nxt |= {d for d, _, _ in fwd.get(n, [])}
            nxt |= {s for s, _, _ in rev.get(n, [])}
        front = nxt - seen; seen |= front
    sub = nx.DiGraph()
    for n in seen:
        for d, a, su in fwd.get(n, []):
            if d in seen:
                sub.add_edge(n, d, amount=a, is_suspicious=su)
    sub.add_node(focal)
    if sub.number_of_nodes() > max_nodes:
        pr = nx.pagerank(sub, alpha=0.85); pr[focal] = 9e9
        keep = sorted(pr, key=pr.get, reverse=True)[:max_nodes]
...
def main():
    ...
    if page == "Overview":
        st.markdown("<div class='page-title'>MuleTrace-IN v2</div>"
                    "<div class='page-sub'>1M+ transactions · IP / geo / device intelligence · 4-model ensemble</div>",
                    unsafe_allow_html=True)
        tx = load_txns()
        c1,c2,c3,c4,c5 = st.columns(5)
        c1.metric("Transactions", f"{len(tx)/1000:.0f}K")
    ...
    # ═════════ INVESTIGATE ═════════
        ...
        tx = load_txns()
        h = tx[(tx.sender_id==sel)|(tx.receiver_id==sel)].sort_values("timestamp", ascending=False).head(100)
        if h.empty: st.info("No transactions.")
        else:
            d = h[["txn_id","timestamp","sender_id","receiver_id","amount","method",
                   "ip_address","device_id","vpn_flag","is_suspicious","pattern"]].copy()
    ...
    # ═════════ MODEL ANALYTICS ═════════
        ...
        mt = pd.DataFrame(metrics["models"])
        mt.columns = ["Model","AUC-ROC","AUC-PR","Precision","Recall","F1"]
